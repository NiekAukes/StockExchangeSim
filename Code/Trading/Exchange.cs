    ...
    public class ECNBroker : IExchange
    {
        ...
        public float GetCheapestSeller(Company cp)
        {
            ...
            for (int i = 0; i < cp.SellOrders.Count; i++)
            {
                try
                {
                    if (cp.SellOrders[i].Stock.Count < 1)
                    {
                        cp.SellOrders.RemoveAt(i);
                        i--;
                        continue;
                    }
            ...
            if (cheapestholder >= 0)
            {
                ...
                lock (holders[cheapestholder].Stocks)
                {
                    if (holders[cheapestholder].Stocks.Count > 0)
                    {
                        ...
                        buyer.stocks[buyer.InterestedCompanies.IndexOf(cp)].Add(
                            holders[cheapestholder].Stocks[0]);
        ...
        public SellOrder sellOrder(List<Stock> stocklist, Company cp, float Limit, int amount = 1)
        {
            for (int i = 0; i < holders.Count; i++)
            {
                while(amount > 0)
                {
                    if (holders[i].bidask.Ask > Limit && stocklist.Count > 0)
                    {
        ...
        public void CheckSortedOrders(Company cp)
        {
            ...
            for (int i = 0; i < cp.SellOrders.Count; i++)
            {
                try
                {
                    lock (cp.SellOrders.SyncRoot)
                    {
                        if (cp.SellOrders[i].LimitPrice < best)
                        {
                            best = cp.SellOrders[i].LimitPrice;
                            sellOrder = cp.SellOrders[i];
                        }
                    }
                }
                catch (Exception e)
        ...
        public void CheckSellOrder(SellOrder Sellorder)
        {
            ...
            for (int i = 0; i < holders.Count; i++)
            {
                while (amount > 0)
                {
                    if (holders[i].bidask.Ask > Limit && Sellorder.Stock.Count > 0)
                    {
